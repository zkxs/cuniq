...
- [memchr](https://crates.io/crates/memchr) is used for performant newline searching using both reads and memory-mapped IO.
...
times for the "Count", "Count (estimate)", "Report" and "Report (Sorted)" categories are bolded.
...
| Command                                                                              | Version   |           Time | Operation        | Implementation Notes                 | Threads | Performance Notes                                                                                           |
| ------------------------------------------------------------------------------------ | --------- | -------------: | ---------------- | ------------------------------------ | ------: | ----------------------------------------------------------------------------------------------------------- |
| `wc -l huge.txt`                                                                     | GNU 8.32  |     0m 19.656s | N/A              | N/A                                  |       1 | Decent baseline for how quickly the file can be traversed.                                                  |
| `sort -u huge.txt \| wc -l`                                                          | GNU 8.32  |    14m 11.935s | Count            | sorting                              |       8 | Loses to dedicated tooling due to sorting the entire input.                                                 |
| `sort huge.txt \| uniq -c > /dev/null`                                               | GNU 8.32  |    28m 46.811s | Report (sorted)  | sorting                              |       1 | Very bad. Better to use `awk` for reports if you are constrained to GNU coreutils.                          |
| `awk '{ a[$0]++ }; END { for (x in a) { print x ": " a[x] } }' huge.txt > /dev/null` | GNU 5.3.2 |    25m 03.082s | Report           | hashtable                            |       1 | Fastest report option in GNU coreutils, but unsurprisingly loses to dedicated tooling.                      |
| `cuniq --memmap --threads=1 huge.txt`                                                | 2.0.0     |     2m 51.260s | Count            | hashtable                            |       1 | Not sorting is cheaper than sorting, so this beats GNU sort easily.                                         |
| `cuniq --memmap huge.txt`                                                            | 2.0.0     |     2m 54.492s | Count            | hashtable                            |      16 | Bottleneck is in hashtable inserts, which cannot be multithreaded.                                          |
| `cuniq < huge.txt`                                                                   | 2.0.0     |     3m 31.441s | Count            | hashtable                            |       1 | stdin cannot not use memmap, and memmap is faster for large files                                           |
| `cuniq --memmap -c huge.txt > /dev/null`                                             | 2.0.0     | **3m 10.261s** | Report           | hashtable                            |       1 | Not sorting is cheaper than sorting, so this beats GNU sort easily.                                         |
| `cuniq --memmap -cs huge.txt > /dev/null`                                            | 2.0.0     | **3m 34.049s** | Report (sorted)  | hashtable                            |       1 | Sorting after dedupe is cheaper than sorting before dedupe, so this beats GNU sort easily.                  |
| `cuniq --memmap --mode=near-exact --threads=1 huge.txt`                              | 2.0.0     |     1m 57.127s | Count            | hashtable, but only stores hash      |       1 | Speedup is from not having to copy any strings.                                                             |
| `cuniq --memmap --mode=near-exact huge.txt`                                          | 2.0.0     | **0m 59.387s** | Count            | hashtable, but only stores hash      |      16 | Speedup is from hashing in background theads and thereby moving the bottleneck to hashtable inserts.        |
| `cuniq --memmap --mode=estimate --threads=1 huge.txt`                                | 2.0.0     |     1m 25.491s | Count (estimate) | HyperLogLog estimate w/ 0.74% error  |       1 | Speedup is from constant-sized 64 KiB memory usage.                                                         |
| `cuniq --memmap --mode=estimate huge.txt`                                            | 2.0.0     | **0m 09.038s** | Count (estimate) | HyperLogLog estimate w/ 0.74% error  |      16 | Speedup is from fully parallelizing, so disk reads become the only remaining bottleneck.                    |
| `sortuniq < huge.txt \| wc -l`                                                       | 0.3.0     |    12m 09.839s | Count            | hashtable                            |       1 | Struggles to outperform even `sort -u` due to several missed optimizations.                                 |
| `sortuniq -c < huge.txt > /dev/null`                                                 | 0.3.0     |    11m 11.184s | Report           | hashtable                            |       1 | Outperforms `awk`, but fails to outperform all other dedicated tooling due to several missed optimizations. |
| `runiq --filter=simple huge.txt \| wc -l`                                            | 2.0.0     |    12m 01.224s | Count            | hashtable                            |       1 | Struggles to outperform even `sort -u` due to several missed optimizations.                                 |
| `runiq --filter=quick huge.txt \| wc -l`                                             | 2.0.0     |     7m 19.354s | Count            | hashtable, but only stores hash      |       1 | Outperforms `sort -u`, but leaves performance on the table from missed optimizations.                       |
| `runiq --filter=compact huge.txt \| wc -l`                                           | 2.0.0     |    13m 27.934s | Count (estimate) | Bloom filter estimate w/ 0.00% error |       1 | Below average time performance but above average memory performance with a remarkably low error rate.       |
| `huniq < huge.txt \| wc -l`                                                          | 2.7.0     |     6m 04.595s | Count            | hashtable, but only stores hash      |       1 | Fairly competitive performance due to minimal missed optimizations.                                         |
| `huniq -c < huge.txt > /dev/null`                                                    | 2.7.0     |     9m 31.972s | Report           | hashtable                            |       1 | Outperforms `awk`, but leaves performance on the table from missed optimizations.                           |
| `huniq -cs < huge.txt > /dev/null`                                                   | 2.7.0     |     9m 38.593s | Report (sorted)  | hashtable                            |       1 | Outperforms `awk`, but leaves performance on the table from missed optimizations.                           |

The commands that are noted as "only stores hash" are in theory vulnerable to hash collisions, but in practice with the
64-bit hashes they're using it would be extraordinarily unlikely to see incorrect results so I'm including them in the
"Count" category.
